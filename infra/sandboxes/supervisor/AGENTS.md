...
## API Rules
- Never expose internal error details or stack traces in responses — return generic error messages
- Validate all input at API boundaries — reject invalid requests early with 400
- Every endpoint requiring auth must be tested unauthenticated → 401
- Never commit secrets — environment variables only, never hardcoded values

## Definition of Done (API)
- [ ] Happy path test passes
- [ ] Auth guard tested: unauthenticated → 401
- [ ] Input validation tested: malformed input → 400
